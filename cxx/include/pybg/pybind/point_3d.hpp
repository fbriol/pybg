...
auto bind_point_3d(nanobind::module_ &m, const char *const name) -> void {
  nanobind::class_<Point<T>>(m, name)
      ...
      .def_prop_rw(
          "z", [](const Point<T> &self) { return self.template get<2>(); },
          [](Point<T> &self, T z) { self.template set<2>(z); })
...
auto instantiate_point_3d(nanobind::module_ &m) -> void {
  bind_point_3d<double, Point>(m, "PointFloat64");
  bind_point_3d<float, Point>(m, "PointFloat32");
}
